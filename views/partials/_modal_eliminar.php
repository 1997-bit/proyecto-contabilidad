<?php $csrf = SessionHelper::generarCsrf(); ?>
<dialog id="dlg-eliminar" onclick="if (event.target === this) { App.cerrarEliminar(); }">
  ...
  <form method="POST" action="<?= BASE_URL ?>/colaborador/eliminar" id="form-eliminar">
    ...
    <div style="display:flex;gap:8px">
      <button type="submit" id="btn-confirmar-eliminar" disabled
        style="background:#ef0606;color:#fff;border:none;padding:4px 12px;cursor:pointer">
        Confirmar eliminación
      </button>
      <button type="button" onclick="App.cerrarEliminar()">Cancelar</button>
    </div>
  </form>
</dialog>
